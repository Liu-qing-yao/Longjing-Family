<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		<!-- 输入框，仅在可输入模式下使用 -->
		<input  
			v-if="showInput"  class="input"  placeholder-style="color: #000;"
		...
		<span 
			@click="onUpperClick" 
			class="iconfont icon-ebae1 right-arrow" 
...
<style scoped lang="less">
	...
	@arrowWidth: 30upx;            //右边的小三角按钮区域的宽度
	.placeholder11{
		color: red; top: 10px;
	}
	.show-box{
		...
		/****** 列表框部分样式 *****/
		.list-container{
			position: absolute; width: 100%; left: 0px; top: 50px;
